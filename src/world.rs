...
use crate::particle::{Particle, net_acceleration};
...
impl World {
    ...
    pub fn update(&mut self, dt: f64) {
        ...
        let mut dt_lock = self.dt.write().unwrap();
        *dt_lock = dt;
        drop(dt_lock);
        
        // main thread starts processing which starts worker threads also as barrier will be unlocked.
        process_particles(&self.barrier, &self.particles, &self.dt, 0, self.num_threads);
    }

    /// Add a new [`Particle`] to the world.
    pub fn create_particle(&mut self, position: DVec2, velocity: DVec2, mass: f64) {
        self.particles.write().unwrap().push(Particle { id: self.count, velocity, position, mass });
        self.count += 1;
        println!("Particle {} added to world", self.count);
    }
    ...
    fn init_worker_threads(&mut self, num_threads: usize) {
        for thread_id in 1..num_threads {
            ...
            self.threads.push(thread::spawn(move || loop { process_particles(&barrier, &particles, &dt, thread_id, num_threads); } )) 
        }
    }
}

fn process_particles(barrier: &Arc<Barrier>, particles: &Arc<RwLock<Vec<Particle>>>, dt: &Arc<RwLock<f64>>, thread_id: usize, num_threads: usize) {
    ...
    let accelerations: Vec<DVec2> = particles_read_lock
        .iter()
        .skip(thread_id)
        .step_by(num_threads)
        .map(|particle| net_acceleration(particle, &particles_read_lock))
        .collect();
